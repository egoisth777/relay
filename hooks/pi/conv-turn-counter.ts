 ...
 * reminder to persist conversation state via the conversate skill. `content`
 ...
 *
 * Install locations (one file, copied by scripts/install.py):
 *   - pi:  <project>/.pi/extensions/conv-turn-counter.ts
 *   - omp: <project>/.omp/hooks/pre/conv-turn-counter.ts
 */
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("before_agent_start", async () => {
    ...
    if (!existsSync(join(process.cwd(), ".conversate"))) {
      return;
    }
    userTurns += 1;
