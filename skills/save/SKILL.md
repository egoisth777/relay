---
name: save
description: Save or checkpoint the current conversation as a resumable record in the Conversation database.
...
5. The CLI writes the TOML markdown under `~/.conversate/convs/`, reconciles refs when present, and rebuilds `~/.conversate/index.jsonl`.
