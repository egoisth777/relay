---
name: sidekick
description: Branch the current conversation into a sidekick record in the Relay archive.
disable-model-invocation: false
argument-hint: "[parent] [topic]"
---

Run the Relay `relay:sidekick` flow. Plugin source is this repo. The installed CLI lives under the Plugin installation root (`~/.relay/` by default) and writes records to the Relay archive (`~/.relay/convs/`).
...
Only after the common sidekick command needs advanced branch behavior, read `~/.relay/references/branching.md`. Examples: probe-vs-sidekick intent, same-session organization, stable scripted ids, or parent-active exceptions.
