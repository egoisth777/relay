...
Run the Relay `relay:resume` flow. Plugin source is this repo. The installed CLI lives under the Plugin installation root (`~/.relay/` by default) and reads records from the Relay archive (`~/.relay/convs/`).
...
2. Resolve the target with:
   `~/.relay/bin/relay search "<id-or-query>"`
...
Only after search/show needs branch or advanced behavior, read `~/.relay/references/resume.md`. Examples: ambiguous linked conversations, branch digests, search-cascade troubleshooting, or resolver details not covered above.
