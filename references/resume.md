...
then Semble over `.conversate/convs` bodies. It uses installed `semble` automatically, can
use `uvx semble` when `CONV_USE_UVX_SEMBLE=1`, and otherwise falls back to built-in body
scoring. If `fff` is available, prefer it manually for the filename layer; keep the same
...
2. `## user-instructions`: **adopt these as standing behavior** for the resumed session
   (constraints, workflow preferences, tone the user set).
3. `## dict`: ubiquitous language — the agreed terms, first.
